...
CFLAGS="-std=c23 -O3 -static -Wall -Wextra -Werror -pedantic \
        ...
        -Wmissing-prototypes -Wstrict-prototypes -Wformat=2 -Wundef -fPIE"
...
SOURCES=("./src/tsukika/android_binaries/c/compileTest/test.c" "./src/tsukika/android_binaries/c/initStateModuleRunner/main.c")
SOURCE_BUILD_OUTPUT_PATH=("./src/tsukika/android_binaries/c/compileTest/hawksnext" "./src/tsukika/android_binaries/c/initStateModuleRunner/initStateModuleRunner")
SKIPSIGN=""
OTA_MANIFEST_URL=""
SDK=""
CC=""
...
for args in "$@"; do
    ...
    case "${lowerCaseArgument}" in
        ...
        ;;
        "headertest"|"initmodules")
            [ "${lowerCaseArgument}" == "headertest" ] && SOURCE_INDEX=0
            [ "${lowerCaseArgument}" == "initmodules" ] && {
                HEADER_SOURCES="$HEADER_SOURCES ./src/include/tsukika_modules.c ./src/include/tsukika_props.c"
                SOURCE_INDEX=1
            }
            buildTargets;
        ;;
        "all")
            for i in $(seq 0 1); do
                SOURCE_INDEX="$i"
                [ "$i" == "1" ] && HEADER_SOURCES="$HEADER_SOURCES ./src/include/tsukika_modules.c ./src/include/tsukika_props.c"
                buildTargets
            done
        ;;
    esac
