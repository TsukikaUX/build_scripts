...
[ "$TARGET_REMOVE_NONE_SECURITY_OPTION" == "true" ] && changeXMLValues "config_hide_none_security_option" "true" "${DECODEDAPKTOOLPATHS[0]}/res/values/bools.xml"
...
if [ "$CUSTOM_WALLPAPER_RES_JSON_GENERATOR" == "true" ]; then
	...
	echo -e "- Please check \" ${DECODEDAPKTOOLPATHS[3]}/raw/resources_info.json\" if you're concerned about issues.\e[0;37m"
fi
...
if [ "$TARGET_BUILD_CUSTOMIZE_SETUP_WIZARD_STRINGS" == "true" ]; then
	xmlstarlet sel -t -v "//string[@name='intro_upper_title']" ${DECODEDAPKTOOLPATHS[10]}/res/values/strings.xml | grep -q '^".*"$' && \
		xmlstarlet ed -L -u '//string[@name="intro_upper_title"]' -v "\"${TARGET_BUILD_SETUP_WIZARD_INTRO_TEXT}\"" ${DECODEDAPKTOOLPATHS[10]}/res/values/strings.xml || \
		xmlstarlet ed -L -u '//string[@name="intro_upper_title"]' -v "${TARGET_BUILD_SETUP_WIZARD_INTRO_TEXT}" ${DECODEDAPKTOOLPATHS[10]}/res/values/strings.xml
